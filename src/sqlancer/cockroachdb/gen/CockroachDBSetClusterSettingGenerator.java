...
public final class CockroachDBSetClusterSettingGenerator {
    ...
    private enum CockroachDBClusterSetting {
        COMPATOR_ENABLED("compactor.enabled    ", CockroachDBSetSessionGenerator::onOff), BUFFER_INCREMENT(
                ...
                (g) -> "'" + Randomly.getUncachedDouble() + "'"), BACKPRESSURE_RANGE_SIZE_MULTIPLIER(
                        ...
                        (g) -> Randomly.getNotCachedInteger(0, Integer.MAX_VALUE)), RANGE_DESCRIPTOR_CACHE_SIZE(
                                ...
                                (g) -> Randomly.getNonCachedInteger()), SQL_DEFAULTS_VECTORIZE_ROW_COUNT_THRESHOLD(
        ...
        MERGE_JOINS_ENABLED("sql.distsql.merge_joins.enabled",
                CockroachDBSetSessionGenerator::onOff), PARALLEL_SCANS_ENABLED("sql.parallel_scans.enabled",
                        CockroachDBSetSessionGenerator::onOff), SQL_QUERY_CACHE_ENABLED("sql.query_cache.enabled",
                                CockroachDBSetSessionGenerator::onOff), SQL_STATS_HISTOGRAM_COLLECTION_ENABLED(
