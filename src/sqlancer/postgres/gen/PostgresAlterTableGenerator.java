...
public class PostgresAlterTableGenerator
     ...
     * errors.add("cannot drop column"); errors.add("cannot drop inherited column"); break; case ALTER_COLUMN_TYPE:
     * alterColumn(randomTable, sb); if (Randomly.getBoolean()) { sb.append(" SET DATA"); } sb.append(" TYPE ");
     * PostgresDataType randomType = PostgresDataType.getRandomType(); PostgresCommon.appendDataType(randomType, sb,
     * false, generateOnlyKnown, opClasses); // TODO [ COLLATE collation ] [ USING expression ]
     * errors.add("cannot alter type of a column used by a view or rule");
     * errors.add("cannot convert infinity to numeric"); errors.add("is duplicated");
     * errors.add("cannot be cast automatically"); errors.add("is an identity column");
     * errors.add("identity column type must be smallint, integer, or bigint"); errors.add("out of range");
     ...
     * errors.add("argument of CHECK must be type boolean"); errors.add("operator does not exist");
     * errors.add("must be type"); errors.add("You might need to add explicit type casts");
     * errors.add("cannot cast type"); errors.add("foreign key constrain"); errors.add("division by zero");
     * errors.add("value too long for type character varying"); errors.add("cannot drop index");
     * errors.add("cannot alter inherited column"); errors.add("must be changed in child tables too");
     ...
     * errors.add("cannot alter type of a column used by a generated column"); break; case
     * ALTER_COLUMN_SET_DROP_DEFAULT: alterColumn(randomTable, sb); if (Randomly.getBoolean()) {
     * sb.append("DROP DEFAULT"); } else { sb.append("SET DEFAULT "); sb.append(PostgresVisitor.asString(
     * PostgresExpressionGenerator.generateExpression(globalState, randomColumn.getType())));
     * errors.add("is out of range"); errors.add("but default expression is of type"); errors.add("cannot cast"); }
     * errors.add("is a generated column"); errors.add("is an identity column"); break; case ALTER_COLUMN_SET_DROP_NULL:
     * alterColumn(randomTable, sb); if (Randomly.getBoolean()) { sb.append("SET NOT NULL");
     * errors.add("contains null values"); } else { sb.append("DROP NOT NULL"); errors.add("is in a primary key");
     * errors.add("is an identity column"); } break; case ALTER_COLUMN_SET_STATISTICS: alterColumn(randomTable, sb);
     * sb.append("SET STATISTICS "); sb.append(r.getInteger(0, 10000)); break; case ALTER_COLUMN_SET_ATTRIBUTE_OPTION:
     * alterColumn(randomTable, sb); sb.append(" SET("); List<Attribute> subset =
     * Randomly.nonEmptySubset(Attribute.values()); int j = 0; for (Attribute attr : subset) { if (j++ != 0) {
     * sb.append(", "); } sb.append(attr.val); sb.append("="); sb.append(Randomly.fromOptions(-1, -0.8, -0.5, -0.2,
     * -0.1, -0.00001, -0.0000000001, 0, 0.000000001, 0.0001, 0.1, 1)); } sb.append(")"); break; case
     * ALTER_COLUMN_RESET_ATTRIBUTE_OPTION: alterColumn(randomTable, sb); sb.append(" RESET("); subset =
     * Randomly.nonEmptySubset(Attribute.values()); j = 0; for (Attribute attr : subset) { if (j++ != 0) {
     * sb.append(", "); } sb.append(attr.val); } sb.append(")"); break; case ALTER_COLUMN_SET_STORAGE:
     * alterColumn(randomTable, sb); sb.append("SET STORAGE "); sb.append(Randomly.fromOptions("PLAIN", "EXTERNAL",
     * "EXTENDED", "MAIN")); errors.add("can only have storage"); errors.add("is an identity column"); break; case
     * ADD_TABLE_CONSTRAINT: sb.append("ADD "); sb.append("CONSTRAINT ").append(r.getAlphabeticChar()).append(" ");
     * PostgresCommon.addTableConstraint(sb, randomTable, globalState, errors); errors.add("already exists");
     * errors.add("multiple primary keys for table"); errors.add("could not create unique index");
     * errors.add("contains null values"); errors.add("cannot cast type");
     ...
     * sb.append(randomTable.getRandomIndex().getIndexName()); errors.add("is not a unique index");
     ...
     * errors.add("which is part of the partition key"); break; case VALIDATE_CONSTRAINT:
     * sb.append("VALIDATE CONSTRAINT asdf"); errors.add("does not exist"); // FIXME select constraint break; case
     * DISABLE_ROW_LEVEL_SECURITY: sb.append("DISABLE ROW LEVEL SECURITY"); break; case ENABLE_ROW_LEVEL_SECURITY:
     ...
     * sb.append("NO FORCE ROW LEVEL SECURITY"); break; case CLUSTER_ON: sb.append("CLUSTER ON ");
     * sb.append(randomTable.getRandomIndex().getIndexName()); errors.add("cannot cluster on");
     * errors.add("cannot mark index clustered in partitioned table"); errors.add("not valid"); break; case
     * SET_WITHOUT_CLUSTER: sb.append("SET WITHOUT CLUSTER");
     * errors.add("cannot mark index clustered in partitioned table"); break; case SET_WITH_OIDS:
     * errors.add("is an identity column"); sb.append("SET WITH OIDS"); break; case SET_WITHOUT_OIDS:
     * sb.append("SET WITHOUT OIDS"); break; case SET_LOGGED_UNLOGGED: sb.append("SET ");
     * sb.append(Randomly.fromOptions("LOGGED", "UNLOGGED")); errors.add("because it is temporary");
     * errors.add("to logged because it references unlogged table");
     * errors.add("to unlogged because it references logged table"); break; case NOT_OF:
     * errors.add("is not a typed table"); sb.append("NOT OF"); break; case OWNER_TO: sb.append("OWNER TO "); // TODO:
     * new_owner sb.append(Randomly.fromOptions("CURRENT_USER", "SESSION_USER")); break; case REPLICA_IDENTITY:
     * sb.append("REPLICA IDENTITY "); if (Randomly.getBoolean() || randomTable.getIndexes().isEmpty()) {
     * sb.append(Randomly.fromOptions("DEFAULT", "FULL", "NOTHING")); } else { sb.append("USING INDEX ");
     * sb.append(randomTable.getRandomIndex().getIndexName()); errors.add("cannot be used as replica identity");
     * errors.add("cannot use non-unique index"); errors.add("cannot use expression index");
     * errors.add("cannot use partial index"); errors.add("cannot use invalid index"); } break; default: throw new
     * AssertionError(a); } }
     *
